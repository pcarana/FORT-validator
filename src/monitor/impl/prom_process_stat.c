...
prom_process_stat_file_new(const char *path)
{
	int pid;
	char buf[50];
...
int
prom_process_stat_file_destroy(prom_process_stat_file_t *self)
{
	int error;
...
prom_process_stat_t *
prom_process_stat_new(prom_process_stat_file_t *stat_f)
{
	prom_process_stat_t *self;

	self = malloc(sizeof(prom_process_stat_t));
	if (self == NULL) {
		pr_enomem();
		return NULL;
	}

	self->comm = malloc(128);
	if (self->comm == NULL) {
		free(self);
		pr_enomem();
		return NULL;
...
int
prom_process_stat_destroy(prom_process_stat_t *self)
{
	if (self == NULL)
		return 0;
 ...
 */
int
prom_process_stats_init(void)
{
	/* /proc/[pid]stat cutime + cstime / 100 */
	prom_process_cpu_seconds_total = prom_gauge_new(
	    "process_cpu_seconds_total",
	    "Total user and system CPU time spent in seconds.", 0, NULL);

	/* /proc/[pid]/stat Field 23 */
	prom_process_virtual_memory_bytes = prom_gauge_new(
	    "process_virtual_memory_bytes", "Virtual memory size in bytes.", 0,
	    NULL);

	prom_process_start_time_seconds = prom_gauge_new(
	    "process_start_time_seconds",
	    "Start time of the process since unix epoch in seconds.", 0,
	    NULL);
...
/*
 * FIXME (now) Add destroy method (maybe 'prom_process_stats_destroy'), it must
 * destroy:
