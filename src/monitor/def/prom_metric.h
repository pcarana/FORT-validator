...
#ifndef SRC_MONITOR_DEF_PROM_METRIC_H
#define SRC_MONITOR_DEF_PROM_METRIC_H
...
/*
 * @brief Returns a prom_metric_sample_t*. The order of label_values is
 *        significant.
 *
 ...
 */
prom_metric_sample_t *prom_metric_sample_from_labels(prom_metric_t *,
    const char **);

/*
 * @brief Returns a prom_metric_sample_histogram_t*. The order of label_values
 * is significant.
 *
 * You may use this function to cache metric samples to avoid sample lookup.
 * Metric samples are stored in a hash map with O(1) lookups in average case;
...
prom_metric_sample_histogram_t *prom_metric_sample_histogram_from_labels(
    prom_metric_t *, const char **);

#endif /* SRC_MONITOR_DEF_PROM_METRIC_H */
