...
#ifndef SRC_MONITOR_DEF_PROM_HISTOGRAM_BUCKETS_H
#define SRC_MONITOR_DEF_PROM_HISTOGRAM_BUCKETS_H
 ...
 * @param bucket The first bucket. A variable number of bucket values may be
 *        passed. This quantity MUST equal the value passed as count.
 * @return The constructed prom_histogram_buckets_t*
 */
prom_histogram_buckets_t *prom_histogram_buckets_new(size_t, double, ...);

/*
 * @brief the default histogram buckets: .005, .01, .025, .05, .1, .25, .5, 1,
 *        2.5, 5, 10
 */
 ...
 * @return The constructed prom_histogram_buckets_t*
 */
prom_histogram_buckets_t *prom_histogram_buckets_linear(double, double, size_t);

/*
 * @brief Construct an exponentially sized prom_histogram_buckets_t*
 * @param start The first inclusive upper bound. The value MUST be greater than
 *        0.
 * @param factor The factor to apply to the previous upper bound to produce the
 *        next upper bound. The value MUST be greater than 1.
 * @param count The total number of buckets. The final +Inf bucket is not
 *        counted and not included. The value MUST be greater than or equal to 1
 * @return The constructed prom_histogram_buckets_t*
 */
prom_histogram_buckets_t *prom_histogram_buckets_exponential(double, double,
    size_t);

/*
 * @brief Destroy a prom_histogram_buckets_t*. Self MUST be set to NULL after
...
#endif /* SRC_MONITOR_DEF_PROM_HISTOGRAM_BUCKETS_H */
