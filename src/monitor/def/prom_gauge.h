...
#ifndef SRC_MONITOR_DEF_PROM_GAUGE_H
#define SRC_MONITOR_DEF_PROM_GAUGE_H
 ...
 * @param label_key_count The number of labels associated with the given metric.
 *        Pass 0 if the metric does not require labels.
 * @param label_keys A collection of label keys. The number of keys MUST match
 *        the value passed as label_key_count. If no labels are required, pass
 *        NULL. Otherwise, it may be convenient to pass this value as a literal.
 * @return The constructed prom_guage_t*
 *
 *     // An example with labels
 *     prom_gauge_new("foo", "foo is a gauge with labels", 2,
 *         (const char**) { "one", "two" });
 *
...
/*
 * @brief Destroys a prom_gauge_t*. You must set self to NULL after destruction.
 ...
 */
int prom_gauge_destroy(prom_gauge_t *);

/*
 * @brief Increment the prom_gauge_t* by 1.
 * @param self The target  prom_gauger_t*
 * @param label_values The label values associated with the metric sample being
 *        updated. The number of labels must match the value passed to
 *        label_key_count in the gauge's constructor. If no label values are
 *        necessary, pass NULL. Otherwise, It may be convenient to pass this
 *        value as a literal.
 * @return A non-zero integer value upon failure
 * *Example*
 *
 ...
 */
int prom_gauge_inc(prom_gauge_t *, const char **);
 ...
 */
int prom_gauge_dec(prom_gauge_t *, const char **);
 ...
 */
int prom_gauge_add(prom_gauge_t *, double, const char **);

/*
 * @brief Subtract the value to the prom_gauge. A non-zero integer value will be
 *        returned on failure.
 * @param self The target prom_gauge_t*
 * @param r_value The double to add to the prom_gauge_t passed as self.
 * @param label_values The label values associated with the metric sample being
 ...
 */
int prom_gauge_sub(prom_gauge_t *, double, const char **);

/*
 * @brief Set the value for the prom_gauge_t*
 * @param self The target prom_gauge_t*
 * @param r_value The double to which the prom_gauge_t* passed as self will be
 *        set
 * @param label_values The label values associated with the metric sample being
 *        updated. The number of labels must match the value passed to
 *        label_key_count in the gauge's constructor. If no label values are
...
#endif /* SRC_MONITOR_DEF_PROM_GAUGE_H */
